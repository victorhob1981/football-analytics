{{ config(
    ...
    indexes=[
        ...
        {'columns': ['season_count desc', 'competition_count desc', 'team_count desc', 'matches_played desc', 'minutes_played desc', 'career_end_at desc', 'player_id'], 'type': 'btree'},
...
aggregated as (
    select
        ...
        count(distinct competition_sk)::int as competition_count,
        count(distinct season)::int as season_count,
        min(match_date) as career_start_at,
        max(match_date) as career_end_at,
