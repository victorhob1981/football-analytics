...
from common.services.warehouse_service import _enrich_statistics_semantics
from data_quality_checks import CHECKS


def test_enrich_statistics_semantics_populates_identity():
    ...
    enriched = _enrich_statistics_semantics(load_df, scope_df=scope_df, run_id="run-456")
    ...
    assert row["source_run_id"] == "run-456"


def test_enrich_statistics_semantics_rejects_unresolved_fixture_scope():
    load_df = pd.DataFrame(
        [
            {
                "fixture_id": 16744816,
    ...
    with pytest.raises(RuntimeError, match="Nao foi possivel resolver identidade semantica de statistics"):
        _enrich_statistics_semantics(load_df, scope_df=pd.DataFrame(), run_id="run-456")


def test_data_quality_checks_include_statistics_semantic_guards():
    check_names = {check["check_name"] for check in CHECKS}

    assert "raw_match_statistics_null_competition_key" in check_names
    assert "raw_match_statistics_null_season_label" in check_names
    assert "raw_match_statistics_null_provider_season_id" in check_names
    assert "raw_match_statistics_outside_catalog" in check_names
